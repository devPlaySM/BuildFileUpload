// 비밀번호 변경 후 즉시 로그아웃 및 새로 고침
function reloadFunction() {
	sessionStorage.clear();
    deleteCookie("powergen_loginToken");
    alert("비밀번호 변경이 완료되었습니다.\n원활한 사용을 위해 재로그인 부탁드립니다.");
...
(function($){
	...
	customTemplate.prototype.renderMessage = function (msgData) {
		...
		}else if (msgData.message[0] && msgData.message[0].component && msgData.message[0].component.payload && msgData.message[0].component.payload.template_type == "reloadTemplate") {
			messageHtml = $(this.getChatTemplate("reloadTemplate")).tmpl({
				'msgData': msgData,
				'helpers': this.helpers,
				'extension': this.extension
			});
...
var advancedListTemplate = '<script id="chat_message_tmpl" type="text/x-jqury-tmpl"> \
	...
	// 비밀번호 변경 완료 시 챗봇 재실행 및 재로그인(세션 & 쿠키 삭제)
	var reloadTemplate =
		'<script id="chat_message_tmpl" type="text/x-jqury-tmpl"> \
			<img src onerror="reloadFunction()">\
			{{if msgData.message}} \
                    <li data-time="${msgData.createdOnTimemillis}" id="${msgData.messageId || msgItem.clientMessageId}"\
                        class="{{if msgData.type === "bot_response"}}fromOtherUsers{{else}}fromCurrentUser{{/if}} with-icon quickReplies"> \
                        <div class="buttonTmplContent"> \
		...
		// 무인도 탈출 게임 용 Template
		var islandGameQuickReplyTemplate = 
			'<script id="chat_message_tmpl" type="text/x-jqury-tmpl"> \
				{{if msgData.message[0].component.payload.bgmName}}\
					<img src onerror="stopBgm()">\
					<audio id="islandGameBgm" autoplay  {{if msgData.message[0].component.payload.bgmLoop}} loop {{/if}}>\
						<source src="../bgm/mp3/${msgData.message[0].component.payload.bgmName}.mp3" type="audio/mp3" />\
						<source src="../bgm/wav/${msgData.message[0].component.payload.bgmName}.wav" type="audio/wav" />\
						<source src="../bgm/ogg/${msgData.message[0].component.payload.bgmName}.ogg" type="audio/ogg" />\
					</audio>\
				{{/if}} \
                {{if msgData.message}} \
                    <li data-time="${msgData.createdOnTimemillis}" id="${msgData.messageId || msgItem.clientMessageId}"\
		...
		}else if (tempType === "reloadTemplate") {
